...
class Engine(object):
    def __init__(self, state={}):
        ...
        self.state['device_ids'] = range(torch.cuda.device_count())
    ...
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        if training:
            self.state['output'] = model(input_var)
            # y_pred = torch.sigmoid(self.state['output'])
            self.state['loss'] = criterion(self.state['output'], target_var)
...
class GCNMultiLabelMAPEngine(MultiLabelMAPEngine):
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
        feature_var = torch.autograd.Variable(self.state['feature']).float()    # [batch_size, 3, 448, 448]
        target_var = torch.autograd.Variable(self.state['target']).float()      # [batch_size, 80]
        inp_var = torch.autograd.Variable(self.state['input']).float().detach()  # [batch_size, 80, 300]
        if not training:
            with torch.no_grad():
                self.state['output'] = model(feature_var, inp_var)
                self.state['loss'] = criterion(self.state['output'], target_var)

        # compute output, feature_var:image, inp_var:label-word-embedding
        if training:
            self.state['output'] = model(feature_var, inp_var)
            self.state['loss'] = criterion(self.state['output'], target_var)
